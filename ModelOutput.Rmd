...
The WRWC Modeling Suite predicts spring air temperatures, total summer runoff volumes, "center of mass", total diversions and timing of delivery calls in the Big Wood River Basin, Camas Creek and Silver Creek. 

### Data Inputs

Data is automatically downloaded from the USGS, NRCS and AgriMet (Fairfield and Picabo).
```{r data info, echo=FALSE, out.width="45%"}
kable(usgs_sites[c(1,2,3,5),] %>% dplyr::select(station_nm, huc_cd, begin_date, end_date, abv) %>% `colnames<-`(c("Station", "HUC", "Start", "End", "Abv")), caption = "USGS Sites") 
```

```{r snotel data info, echo=FALSE, out.width="65%"}
...
#update this to auto select the month to compare average to
swe$per <- round(swe[,5]/swe$Feb1,2)*100
da<-format(last_date, "%b %d")

swe %>% mutate(per = cell_spec(per, color = ifelse(per > 75, "blue","red"))) %>% `colnames<-`(c("Site", "Feb1", "Mar1", "Apr1", da, "Per of Avg"))  %>% kable(align=rep('c', 6), escape = FALSE, format = "latex") %>% kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) 
...
These box plots shows the range of volumes that were calculated for each basin. The boxes represent the 25th - 75th percentiles, with the median in the solid line in the middle, circles are outliers. 
...
Mean simulated streamflow is shown in the solid blue line and the 95% confidence interval is shown in the shaded gray area.
...
```{r curtailment summary, echo=FALSE, out.width = '110%', fig.align='center'}
