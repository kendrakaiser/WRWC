...
#Toy data
data <- data.frame(Date = seq(ymd('1980-01-01'), ymd('2000-01-01'), 1))
data$Q <- sin(yday(data$Date)*2*pi/365) + rnorm(length(data$Date))+ 3
#Calculate day of water-year
water_year_begin <- ymd('1980-06-01')
data$DOY <- (yday(data$Date) - yday(water_year_begin)) %% 365 +1
#Summary stats.
data <- group_by(data, DOY) %>% mutate(meanQ=mean(Q), maxQ=max(Q), minQ=min(Q))
#Plot
ggplot(data=data, mapping=aes(x=DOY))+
