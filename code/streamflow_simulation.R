...
tst<- as.data.frame(apply(bwb.flow.s, 1, CI, 0.95)) #this returns effectively three versions of the mean
...
#flow= bwb.wy[bwb.wy$wy == 2020, "bwb.nat.q"]
...
plot(dates, pi[,6], type="n", xlab="Date", ylab ="Flow (cfs)",
     main = "Big Wood Streamflow /n at Stanton Crossing", ylim=c(min(pi[,4]), max(pi[,5])))
...
plot(dates,pi[,12], type="n", xlab="Date", ylab ="Flow (cfs)",
     main = "Silver Creek Streamflow", ylim=c(min(pi[,10]), max(pi[,11])))
...
plot(dates,pi[,18], type="n", xlab="Date", ylab ="Flow (cfs)",
     main = "Camas Creek Streamflow", ylim=c(min(pi[,16]), max(pi[,17])))
