...
write.csv(var, file.path(cd,'March1_vars.csv'))
...
# --------------------------------------------------
# Big Wood at Stanton
hist <- var[var$year < pred.yr & var$year > 1996,] %>% select(bws.vol.nat, bws.wq, gs.swe) 
...
pred.dat<-var[var$year == pred.yr,] %>% select(bws.wq, gs.swe) 
...
hist <- var[var$year < pred.yr,] %>% select(sc.vol.nat, sc.wq, sp.swe, hc.swe) 
...
pred.dat<-var[var$year == pred.yr,] %>% select(sc.wq, sp.swe, hc.swe) 
...
write.csv(output.vol, file.path(cd,"pred.output.vol.mar.csv"),row.names=T)
write.csv(pred.params.vol, file.path(cd,"pred.params.vol.mar.csv"),row.names=T)
...
png(file.path(fig_dir,"model_summary_mar.png"), height = 25*nrow(mod_sum), width = 110*ncol(mod_sum))
grid.table(mod_sum)
dev.off()
...
write.csv(exp(vol.sample), file.path(cd,"vol.sample.march.csv"),row.names=F)
