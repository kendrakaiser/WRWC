...
##### FEB
allDat<-read.csv(file.path(data_dir, 'all_dat_feb.csv')) ## TODO: check the naming convention here and make sure automation with full script works
...
#### March 
allDat<-read.csv(file.path(data_dir, 'all_dat_mar.csv')) ## TODO: check the naming convention here and make sure automation with full script works
vol <- allDat %>% dplyr ::select (year, bwb.vol, bws.vol, cc.vol, sc.vol) %>% pivot_longer(cols=c('bwb.vol', 'bws.vol', 'cc.vol', 'sc.vol'), names_to = 'site', values_to = 'volume')
vol<- vol[!is.na(vol$volume),]
vol<- vol[vol$volume>0,]
vol<-merge(vol, siteIDs, by='site')
...
### April
allDat<-read.csv(file.path(data_dir, 'all_dat_apr.csv')) ## TODO: check the naming convention here and make sure automation with full script works
vol <- allDat %>% dplyr ::select (year, bwb.vol, bws.vol, cc.vol, sc.vol) %>% pivot_longer(cols=c('bwb.vol', 'bws.vol', 'cc.vol', 'sc.vol'), names_to = 'site', values_to = 'volume')
vol<- vol[!is.na(vol$volume),]
vol<- vol[vol$volume>0,]
vol<-merge(vol, siteIDs, by='site')
